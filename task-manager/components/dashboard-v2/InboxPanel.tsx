...
export default function InboxPanel({ userId, onClose }: { userId: string; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'Notification', filter: `userId=eq.${userId}` },
        () => { loadNotifications() })
      .subscribe()
  ...
  const markAsRead = async (notificationId: string) => {
    try {
      const { error } = await supabase.from('Notification').update({ isRead: true }).eq('id', notificationId)
      if (error) throw error
  ...
  const markAllAsRead = async () => {
    try {
      const { error } = await supabase.from('Notification').update({ isRead: true }).eq('userId', userId).eq('isRead', false)
      if (error) throw error
      await loadNotifications()
    } catch (err) {
  ...
  const deleteAllNotifications = async () => {
    try {
      const { error } = await supabase.from('Notification').delete().eq('userId', userId)
      if (error) throw error
      await loadNotifications()
    } catch (err) {
      ...
      >
        <div className="inbox-header">
          ...
          <div className="inbox-header-actions">
            {unreadCount > 0 && (
              <button onClick={markAllAsRead} className="inbox-mark-all-read">Mark all as read</button>
            )}
            {notifications.length > 0 && (
              <button onClick={handleDeleteAllClick} className="inbox-delete-all">Delete All</button>
            )}
            <button onClick={onClose} className="inbox-close-btn">✕</button>
          </div>
        </div>
                ...
                <div
                  ...
                  onClick={() => {
                    if (notification.type === 'team_invite' && notification.link) {
                      ...
                      if (inviteId) setSelectedInviteId(inviteId)
                    }
                  }}
                  style={{ cursor: notification.type === 'team_invite' ? 'pointer' : 'default' }}
                >
                  <div className="inbox-item-icon">{getNotificationIcon(notification.type)}</div>
                  <div className="inbox-item-content">
                    ...
                    <div className="inbox-item-actions" onClick={(e) => e.stopPropagation()}>
                      {!notification.isRead && (
                        <button onClick={() => markAsRead(notification.id)} className="inbox-action-btn inbox-action-read">
                          Mark as read
                        </button>
                      )}
                      <button onClick={() => handleDeleteClick(notification.id)} className="inbox-action-btn inbox-action-delete">
                        Delete
                      </button>
                    </div>
                  </div>
