...
export default function TeamInviteModal({ inviteId, userId, onClose, onAccept, onDecline }: TeamInviteModalProps) {
  ...
  const loadInviteDetails = async () => {
    try {
      ...
      if (data) {
        setDetails({ teamName: data.team_name, ownerEmail: data.owner_email, role: data.invite_role })
      } else {
  ...
  const handleAccept = async () => {
    try {
      setAccepting(true)
      const { error } = await supabase.rpc('accept_team_invite', { invite_id: inviteId, user_id: userId })
      if (error) throw error
      onAccept()
    } catch (err) {
  ...
  const handleDecline = async () => {
    try {
      setDeclining(true)
      const { error } = await supabase.rpc('decline_team_invite', { invite_id: inviteId, user_id: userId })
      if (error) throw error
      onDecline()
    } catch (err) {
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="team-invite-actions">
              <button onClick={handleDecline} disabled={declining || accepting} className="team-invite-btn team-invite-decline">
                {declining ? 'Declining...' : 'Decline'}
              </button>
              <button onClick={handleAccept} disabled={accepting || declining} className="team-invite-btn team-invite-accept">
